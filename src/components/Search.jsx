import {useState} from 'react';
  ...
  function SearchMovies(){
    ...
    const searchMovies = async(e) =>{
        e.preventDefault();
        setSearching(true);
        const url = `http://www.omdbapi.com/?&apikey=9356f9d0&s=${query}&type="movie"`;
         try{
        ...
        return (
            <div className="container mx-auto pt-6">
               ...
               <form role='form'  className="w-full h-10 pl-3 pr-2 bg-white border rounded-full flex justify-between items-center relative" onSubmit={searchMovies}>
                 <input type="text" name="query" placeholder="Search movies by name..."
                         className="appearance-none w-full outline-none focus:outline-none active:outline-none" value={query} onChange={(e) =>setQuery(e.target.value)}/>
                     ...
                     <div className="bg-white rounded-lg overflow-hidden shadow-xl my-8 py-4">
                      ...
                      <div className="p-4">
                          <p className="font-medium text-lg">Title: <span className="font-normal text-base leadin-relaxed">{movie.Title}</span></p>
                           <p className="font-medium text-lg">Year of Release: <span className="font-normal text-base">{movie.Year}</span></p></div>
                            </div>
                          </div> 
                        )))}
                    </div>
